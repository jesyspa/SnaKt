...
class ProgramConverter(
    ...
    private fun linearizePure(name: SymbolicName, signature: CompleteFunctionSignature, callGraph: PureCallGraph) {
        ...
        linearizedBodyResolver.storeFunction(
            name,
            signature.toViperFunction(typeResolver, linearized, callGraph.mayRecurse(name)),
        )
    }
    ...
    fun linearizeAll() {
        ...
        val callGraph = PureCallGraph.of(pure.associate { it.key to it.value }, convertedBodyResolver)
        pure.forEach { linearizePure(it.key, it.value, callGraph) }
