 ...
 * ```
 *
 * The empty domain that gives predicate instances their type. Silver's `PredicateInstancePlugin`
 * refuses to run on a program that does not declare it. The declaration is separate from the type:
 * [PredicateInstanceType] is what to write where the type is used, and this is what the plugin looks
 * for.
 */
...
object PredicateInstancesWellFoundedOrder :
    ...
    /**
     * The instantiation every application in this domain carries. Passing the identity map or an
     * empty one instead type-checks and verifies without complaint, but the axioms then constrain
     * the wrong instance of `decreasing`/`bounded` and every termination proof goes through
     * vacuously.
     */
 ...
 *
 * A measure over a nullable structure needs this as much as the order on predicate instances: at the
 * recursive call that reaches the end of the structure the two instances are unrelated, and it is
 * the leading nullity component that has to decrease, from `true` to `false`.
 */
