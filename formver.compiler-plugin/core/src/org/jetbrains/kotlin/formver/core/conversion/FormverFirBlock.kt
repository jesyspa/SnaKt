...
/**
 * Drops the attributes that a specification binder cannot restate.
 *
 * `@Unique` and `@Borrowed` are rejected on a type argument, so a `postconditions` binder cannot spell out the
 * uniqueness and locality of the value it stands for.
 */
private fun ConeKotlinType.withoutBinderAttributes(): ConeKotlinType =
...
private fun FirAnonymousFunction.extractFormverReturnVar(returnType: ConeKotlinType): FirValueParameterSymbol {
    ...
    if (declaredType.withoutBinderAttributes() != returnType.withoutBinderAttributes())
