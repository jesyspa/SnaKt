...
/**
 * Lowers a call to the primary constructor of [symbol]'s class into an allocation, writes of the
 * parameters backing properties, and folds of the class and supertype unique predicates. Returns null if
 * the constructed type has no class embedding, and so no unique predicate to establish.
 *
 * A bodyless method promising only `acc(C_unique(ret), write)` leaves the predicate's snapshot
 * unconstrained, so a heap-dependent function reading through it is unrelated to the arguments.
 * Allocating and folding here ties the snapshot to the heap the arguments were written into.
 */
fun StmtConversionContext.insertPrimaryConstructorCall(
    ...
    val written = symbol.valueParameterSymbols.zip(valueArgs).mapNotNull { (param, arg) ->
        parameterMatching[param]?.let { it to arg }
    }
    val initializers = written.map { (property, arg) ->
    ...
    val initialized = written.mapTo(mutableSetOf()) { it.first }
...
/**
 * The type invariants of the backing fields of [classType] that a constructor call does not establish
 * from its arguments, as assumptions about [obj]. [initialized] are the properties a constructor
 * parameter was written into.
 *
 * A property is left out of [initialized] when nothing runs that would give it a value: neither a class
 * body initializer nor a superclass constructor's arguments are converted. The field then holds an
 * arbitrary reference, and its Kotlin type has to be assumed for the fold to go through. That is a fact
 * the Kotlin type system guarantees, and no more than the bodyless method this replaces assumed.
 */
private fun StmtConversionContext.residualPredicateAssumptions(
    ...
    for (property in typeResolver.lookupClassProperties(classType.name)) {
        ...
        val field = (property.getter as? BackingFieldGetter)?.field ?: continue
        if (field.accessPolicy == AccessPolicy.ALWAYS_WRITEABLE) continue
